...
class HtmlResultRenderer : SuitableResultRenderer() {
    override fun isSuitFor(element: Element) = element.localName == "div"

    override fun successReported(event: VerifySuccessEvent<Expected, Actual>) = event.element.let { el ->
        el.appendSister(
            template(
    ...
    override fun failureReported(event: VerifyFailureEvent<Expected>) = event.element.let { el ->
        el.appendSister(
            when (event.fail) {
...
class MdResultRenderer : SuitableResultRenderer() {
    override fun isSuitFor(element: Element) = element.localName != "div"
...
class FirsSuitableResultRenderer(private vararg val renderers: SuitableResultRenderer) :
    VerifyListener<Expected, Actual> {
    override fun successReported(event: VerifySuccessEvent<Expected, Actual>) =
...
abstract class SuitableResultRenderer : VerifyListener<Expected, Actual> {

    abstract fun isSuitFor(element: Element): Boolean

    companion object {
        @JvmStatic
        protected fun template(name: String, messages: List<MessageVerifyResult>) = //language=html
            """
            <div class="mq-check">
